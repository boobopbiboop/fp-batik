...
import {
  GetKainByClassRequest,
  GetKainByClassResponse,
} from "../model/KainModel";
import { KainRepository } from "../repository/KainRepository";
import { Kain } from "../model/KainModel";

export class KainService {
  static async getKainByClass(
    request: GetKainByClassRequest
  ): Promise<GetKainByClassResponse> {
    ...
    if (!fs.existsSync(imagePath)) {
      throw new ResponseError(
        ...
        "Image directory not found for this kain" + imagePath
      );
    }
  ...
  static async getAllKain(): Promise<Kain[]> {
    ...
    const kainDir = process.env.KAIN_PATH as string;
    for (const kain of kains) {
      const imagePath = path.join(kainDir, kain.dirUrl).replace(/\\/g, "/");
      const images = fs.readdirSync(imagePath);
      const randomImage = images[Math.floor(Math.random() * images.length)];
      kain.dirUrl = path.join(imagePath, randomImage).replace(/\\/g, "/");
    }
    return kains;
  }
}
